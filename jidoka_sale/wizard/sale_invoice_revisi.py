...
class SaleAdvancePaymentInvRevisi(models.Model):
    _name = "sale.advance.payment.inv.revisi"
    _description = "Sales Advance Payment Invoice Revisi"
    ...
    def _default_has_down_payment(self):
        if self._context.get('active_model') == 'sc.revisi' and self._context.get('active_id', False):
            sale_order = self.env['sc.revisi'].browse(self._context.get('active_id'))
            return sale_order.order_line_ids.filtered(
    ...
    def _create_invoice(self, order, so_line, amount):
        if (self.advance_payment_method == 'percentage' and amount <= 0.00) or (self.advance_payment_method == 'fixed' and amount <= 0.00):
    ...
    def create_invoices(self):
        sale_orders = self.env['sc.revisi'].browse(self._context.get('active_ids', []))
        ...
        if self.advance_payment_method == 'delivered':
            ...
            sale_line_obj = self.env['sc.revisi.line']
