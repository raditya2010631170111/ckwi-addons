...
class kb_mrp_pembahanan(models.Model):
    ...
    #_description = 'My Group'
# can't two api.model, terakhir yg di run
    @api.model
                ...
                for detail in line.kb_mrp_sawmil_line_ids:
                    ...
                    if check_sq:
                        if check_sq.quantity < 0:
                            check_sq.quantity = detail.hasil_keping
                        else:
                            check_sq.quantity += detail.hasil_keping
                    else:
    ...
    def write(self, vals):
        ...
        if 'product_sawmil_ids' in vals:
            ...
            for line in self.product_sawmil_ids:
                if line.hasil_keping < self.product_uom_qty:
                    total += line.hasil_keping
                else:
                    total = self.product_uom_qty
