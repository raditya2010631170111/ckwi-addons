...
class RekapOrderWizard(models.TransientModel):
    _name = "rekap.order.wizard"
    _description = "Rekap Order Wizard"


    date_from = fields.Date(string='Start Date')
    date_to = fields.Date(string='End Date')
    # report_type = fields.Selection([('In', 'In'), ('Out', 'Out')])
    ...
    def button_export_html_rekap_order(self):
        self.ensure_one()
        action = self.env.ref("jidoka_manufacturing_report.action_rekap_order_html")
        vals = action.sudo().read()[0]
        context = vals.get("context", {})
        if context:
            context = safe_eval(context)
        model = self.env["rekap.report"]
